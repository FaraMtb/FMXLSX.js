...
var fmxlsx = {
    ...
    jsonColumnHeaderRow: function(rowObj) {
       var c = 0;
     for (var p in rowObj) {
              c = this.parse2Int0(c);
    ...
    getBlob: function(workbookObj,jsonColumns,jsondata, options) {
        this.init(options);
        ...
        var wbout = XLSX.write(workbook, { bookType: this.config.fileType, bookSST: true, type: 'binary' });
        
        var blobData = new Blob([this.s2ab(wbout)], { type: "application/octet-stream" });
        return blobData;
    },
    export: function(workbookObj,jsonColumns,jsondata,options) {
             
        saveAs(this.getBlob(workbookObj,jsonColumns,jsondata, options), this.config.fileFullName);
    },
}
